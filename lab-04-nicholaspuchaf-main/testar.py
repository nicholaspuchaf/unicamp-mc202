...
class Task(BaseTask):
    ...
    def teste_teste(self):
        binary = self.compile(binary="mapeamento")
        self.test_cases(binary, self.case_range("teste{}.in", 1, 15))


if __name__ == "__main__":
    args = get_parser().parse_args()
    t = Task(args.c, args.only, args.dry_run)
    if args.test is not None:
        # Quando a flag --test é usada, o binário é compilado e executado
        # apenas para o caso de teste especificado
        t.test_cases(t.compile(binary="mapeamento"), args.test)
    else:
        t.test_units()
