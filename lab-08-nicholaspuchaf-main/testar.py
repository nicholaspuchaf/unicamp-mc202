...
def get_parser():
    ...
    parser.add_argument(
        "-f", action="store_true", help="Forca continuar mesmo que anteriores falhem"
    )
    parser.add_argument(
...
class Task(BaseTask):
    """
    Cada grupo de teste é definido por um método que comeca com teste_.
    Cada teste precisa compilar o binário, seja pelo método compile ou pelo Makefile.
    """

    def teste_pequenos(self):
        global binary
        binary = self.generate_binary(binary=binary)
        self.test_cases(binary, self.case_range("teste{}.in", 1, 10))
